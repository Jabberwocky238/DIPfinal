...
def cv_imshow(img, title='[TEST]', wait=0.5, move=None):
    ...
    return cv2.waitKey(int(wait * 1000))
...
def initialze_flow(img, sobel_size):
    ...
    """
    img = cv2.normalize(img, dst=None, alpha=0.0, beta=1.0,
    ...
    # normalize magnitude
    mag = cv2.normalize(mag, dst=None, alpha=0.0, beta=1.0,
...
def run(img, sobel_size=5, etf_iter=4, etf_size=7,
    ...
    sigma_m=3.0, tau=0.907 , mode=None):
    """
    ...
    result = detect_edge(img, flow, thresh=thresh,
        sigma_c=sigma_c, rho=rho, sigma_m=sigma_m, tau=tau)
    
    if mode == 'improved':
        result = invert_colors(cv2.Canny(result,100,200))
